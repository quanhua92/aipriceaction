...
### Added
- `--latest` flag to `fundamentals rank` and `fundamentals screen` subcommands for period filtering
- `--yearly` flag to `fundamentals rank` and `fundamentals screen` subcommands

### Changed
- Updated all docs (AGENTS.md, skill files) to reflect new flags and version gate >=0.1.43
...
### Changed
- Bump SDK dependency to >=0.1.20

## [0.1.39] - 2026-05-29

### Changed
- Bump SDK dependency to >=0.1.19 (reduced S3 requests, per-ticker timeout, yearly_min_date filter)

### Added
- `--verbose` flag on `aipa analyze` enables SDK debug logging (fetch timing, date ranges, miss counters)

## [0.1.38] - 2026-05-20

### Added
- `aipa --version` flag to print CLI version

## [0.1.37] - 2026-05-20

### Changed
- `_ensure_setup()` now prints a warning instead of blocking with interactive `input()` prompt
...
### Changed
- Require aipriceaction>=0.1.17 for persisted freshness cache, parallel per-day fetching, and cached 404s
