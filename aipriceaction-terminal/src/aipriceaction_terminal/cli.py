...
def run():
    ...
    _shared_parent.add_argument("--no-system-prompt", action="store_true",
        help="Exclude system prompt/persona header from output (only affects analyze and get-ohlcv-data)")
    ...
    wl_sub.add_parser("ls", help="List all watchlists", parents=[_shared_parent])
    p_wl_get = wl_sub.add_parser("get", help="Show tickers for a watchlist", parents=[_shared_parent])
    p_wl_get.add_argument("name", help="Watchlist name")
    p_wl_set = wl_sub.add_parser("set", help="Create/update a custom watchlist", parents=[_shared_parent])
    p_wl_set.add_argument("name", help="Custom watchlist name")
    p_wl_set.add_argument("tickers", nargs="+", help="Ticker symbols")
    p_wl_rm = wl_sub.add_parser("rm", help="Delete a custom watchlist", parents=[_shared_parent])
    ...
    p_fund = sub.add_parser("fundamentals", help="Fundamental data: company info, financial ratios, ranking, screening", parents=[_shared_parent])
    fund_sub = p_fund.add_subparsers(dest="fundamentals_command")

    p_fund_info = fund_sub.add_parser("info", help="Company profile, shareholders, officers", parents=[_shared_parent])
    p_fund_info.add_argument("ticker", help="Ticker symbol")
    p_fund_info.add_argument("--source", default=None)

    p_fund_ratios = fund_sub.add_parser("ratios", help="Financial ratios by period", parents=[_shared_parent])
    ...
    p_fund_rank = fund_sub.add_parser("rank", help="Rank tickers by a fundamental field", parents=[_shared_parent])
    ...
    p_fund_screen = fund_sub.add_parser("screen", help="Screen tickers by fundamental criteria", parents=[_shared_parent])
